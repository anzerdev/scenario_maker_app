...
class FirebaseStorage {
  ...
  Future<void> saveScenario(ScenarioResultModel scenarioResult) async {
    ...
    if (userId != null) { // проверка существования пользователя
      await _db
          .collection(userId)
          .doc(scenarioResult.id)
          .set(scenarioResult.toJSON());
    } else { // что делать, если юзер не существует
  ...
  Stream<List<ScenarioResultModel>> getScenariosStream() {
    ...
    if (userId != null) {
      return _db.collection(userId).snapshots().map( //получаем результат
            (snapshot) => snapshot.docs.map( // преобразуем результат в документ
              (doc) {
                return ScenarioResultModel.fromJSON( // каждый сценарий превращаем в модель результата
                  doc.data(),
                );
              },
